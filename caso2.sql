...
CREATE TABLE pruebas_alumnos(pruebas_id INT REFERENCES pruebas(id), alumnos_rut VARCHAR(20) REFERENCES alumnos(rut));
CREATE TABLE cursos(id SERIAL PRIMARY KEY, nombre_asignatura VARCHAR(75) NOT NULL);
CREATE TABLE alumnos_cursos(cursos_id INT REFERENCES cursos(id), alumnos_rut VARCHAR(20) REFERENCES alumnos(rut));
